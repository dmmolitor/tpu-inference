...
def ref_w4a8_fp8_dynamic(
        x: torch.Tensor,
        w_float: torch.Tensor,  # w_s: torch.Tensor,
...
def return_ref_and_layer_output(layer: torch.nn.Module, batch_size: int = 16):
    ...
    print(
        f"ref_output max: {torch.max(ref_output)}, min: {torch.min(ref_output)}"
    )
    print(
        f"layer_output max: {torch.max(layer_output)}, min: {torch.min(layer_output)}"
    )
